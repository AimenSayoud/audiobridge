...
class AudioBridgeService : LifecycleService() {
    ...
    private fun acquireWakeLock() {
        ...
        wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "AudioBridge::playback").apply {
    ...
    private fun buildNotification(state: BridgeState, bufferMs: Int): Notification {
        ...
        val stop = PendingIntent.getService(
            this, 1, Intent(this, AudioBridgeService::class.java).setAction(ACTION_STOP),
            PendingIntent.FLAG_IMMUTABLE or PendingIntent.FLAG_UPDATE_CURRENT,
        )
    ...
    companion object {
        const val ACTION_START = "dev.audiobridge.app.START"
        const val ACTION_STOP = "dev.audiobridge.app.STOP"
        const val EXTRA_PAIRING = "pairing"

        private const val TAG = "AudioBridgeSvc"
        private const val CHANNEL_ID = "audiobridge.playback"
        private const val NOTIFICATION_ID = 1
        private const val WAKE_LOCK_TIMEOUT_MS = 8L * 60 * 60 * 1000

        fun start(context: Context, pairingUri: String) {
            val intent = Intent(context, AudioBridgeService::class.java)
        ...
        fun stop(context: Context) {
            context.startService(
                Intent(context, AudioBridgeService::class.java).setAction(ACTION_STOP)
            )
        }
    }
}
