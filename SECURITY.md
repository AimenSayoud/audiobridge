...
Tethertone is designed for a phone and a Mac owned by the same person, on a
...
- **Authentication:** a phone must present a 16-byte random token, taken from
  ...
  constant time and stored at `~/.tethertone/token` with mode `0600`.
...
[GitHub security advisories](https://github.com/AimenSayoud/tethertone/security/advisories/new)
